...
| Description | `description` | Full bug report with context, impact, and environment details |
| Components | `components` | ACM component(s) where the bug exists |
...
| Field | Key | Guidance |
|-------|-----|----------|
| Steps to Reproduce | `customfield_10821` | Numbered steps to reproduce the issue |
| Acceptance Criteria | `customfield_10718` | What does "fixed" look like? |
...
| Customer Impact | `customfield_10689` | Escalated (16324), Facing (16325), QE (16326), Reported (16327) |
| Special Handling | `customfield_10670` | compliance-priority, Major Incident, etc. |
| Environment | `environment` | OS, browser, cluster version, infrastructure details |
...
**Description**:
## Problem
When importing a cluster using a kubeconfig file that contains special characters (e.g., `@`, `#`, `%`) in the cluster name or context, the import wizard completes without error but the cluster never appears in the managed clusters list.

## Impact
Users who have non-standard characters in their kubeconfig (common with EKS and custom installations) cannot import clusters via the UI. They must manually edit their kubeconfig first, which is undiscoverable.
